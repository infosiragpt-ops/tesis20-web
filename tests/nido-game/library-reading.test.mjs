...
test('los filtros separan la voz de estudio de las 33 ediciones clásicas', () => {
  assert.equal(searchBooks(BOOKS, '', 'narrated').length, BOOKS.length);
  assert.equal(searchBooks(BOOKS, '', 'reading').length, 33);
  assert.equal(searchBooks(BOOKS, '', 'classic').length, 33);
  assert.equal(searchBooks(BOOKS, 'pulgarcito', 'reading').length, 0);
