...
export const CLASSIC_DIRECTION = {
  ...
  bambi: p([actor('bambi','bambi cervatillo ciervo'), actor('liebre','tambor conejo'), actor('buho','búho'), mother], ['arbol','flores'], 'pajaros'),
  ...
  'patito-feo': p([actor('patito','patito pato patos'), actor('cisne','cisne cisnes'), actor('gato','gato'), actor('gallo','gallina'), mother], ['arbol','flores'], 'arroyo'),
  ...
  heidi: p([actor('heidi','heidi niña'), actor('zapatero','abuelo'), actor('campesina','deta tía'), actor('pastor','pedro'), actor('oveja','cabra cabras')], ['casa','arbol'], 'viento-suave'),
  ...
  'perla-dragon': p([actor('dragon','dragón'), actor('principe','príncipe príncipes hijo'), king], ['perla','barco'], 'viento-suave'),
...
const verbs = [
  ...
  [/^(vol|vuela)/, 'fly'], [/^(nad|sumerg)/, 'swim'], [/^(salt|brinc)/, 'jump'],
  ...
  [/^(trabaj|cos|constru|remend)/, 'build'], [/^(sonri|rió|alegr)/, 'cheer'],
];
...
export function directClassic(item, texts) {
  ...
  const pages = texts.map((x, index) => {
    ...
    const light = /\bnoche\b|oscuridad|anochecer/i.test(x) ? 'night' : item.light || 'day';
